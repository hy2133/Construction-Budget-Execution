...
' 2025.01.14 수정
    ...
    Dim rH As Long, fRow As Long
    Dim minSpc As Integer, curSpc As Integer, vJ As String
...
'선택한 항목 공사 견적의뢰서 만들기===================================================================
  ...
  Next a1
    ...
    ' [수정] 상위 레벨(계층 구조) 유지하며 숨기기 알고리즘
    ' [수정] 내역 시작행(FirstItemRow) 탐색 ("총 공 사 비 계" 아래부터 적용)
    fRow = 4
    For rH = 4 To L2
        If Replace(Sheets(Mywork).Cells(rH, 10).Text, " ", "") = "총공사비계" Then
            fRow = rH + 1 ' 총공사비계 아랫줄부터 숨김 로직 적용
            Exit For
        End If
    Next rH
    
    minSpc = 999
    ' 내역의 끝(L2)부터 첫 시작행(fRow)까지만 역순으로 탐색
    For rH = L2 To fRow Step -1
        vJ = Sheets(Mywork).Cells(rH, 10).Text
        curSpc = 0
        ...
        If Trim(Sheets(Mywork).Cells(rH, 24).Text) <> "" Then
            ' 대상 공종이면 표시하고 레벨 갱신
            Sheets(Mywork).Rows(rH).Hidden = False
            minSpc = curSpc
        Else
            ' 대상 공종이 아니더라도, 더 상위 레벨(들여쓰기가 적음)이면 표시 (계층 구조 보존)
            If curSpc < minSpc And minSpc <> 999 Then
                Sheets(Mywork).Rows(rH).Hidden = False
                minSpc = curSpc
            Else
                Sheets(Mywork).Rows(rH).Hidden = True
            End If
        End If
    Next rH
    ' fRow 위쪽(문서 헤더 4~fRow-1)은 숨기지 않고 그대로 둡니다.
